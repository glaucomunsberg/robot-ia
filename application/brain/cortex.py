...
class Cortex:
    ...
    def add_task(self, func, task_type, priority=3, args=None, kwargs=None) -> None:  # pylint: disable=too-many-arguments too-many-positional-arguments
        ...
        if task_type not in {"SENSOR", "ACTUATOR", "BRAIN", "ACTION", "PROCESS"}:
            raise ValueError(
                f"Type most be one of SENSOR, BRAIN, ACTION, ACTUATOR, PROCESS not {task_type}")
    ...
    def run(self) -> None:
        ...
        if len(self.tasks) == 0:
            ...
            #     print(f"Cortex Keyboard Error: {task_hash}")
            # except ImportError as e:
            #     print(f"Cortex Import Error: {task_hash} -> {e}")
            # except AttributeError as e:
            #     print(f"Cortex Attribute Error: {task_hash} -> {e}")
            # except IndexError as e:
            #     print(f"Cortex Index Error: {task_hash} -> {e}")
            # except KeyError as e:
            #     print(
            #         f"Cortex Key Error: {task_hash} -> {e}")
            #     sys.print_exception(e)
            except Exception as e:  # pylint: disable=broad-except
                print(f"Cortex Error: {task_hash} -> {e} type: {type(e)}")
                sys.print_exception(e)
